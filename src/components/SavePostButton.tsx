...
export default function SavePostButton({ postId }: { postId: string }) {
  ...
  // Which lists already contain this post
  ...
  async function onSave() {
    ...
    await add.mutateAsync({ listIds: chosenIds, postId })
    setOpen(false)
  }
  ...
  async function onCreate(name: string) {
    ...
    const res = await create.mutateAsync({ name: n, kind: 'POSTS' })
      ...
      {open && (
        <div className="absolute right-0 mt-2 z-50 w-72 rounded-xl border bg-white shadow-lg p-3">
          ...
          {!listsQ.isLoading && !listsQ.data?.length && (
            <div className="text-sm text-gray-500">No “Posts” lists yet. Create one below.</div>
          )}
